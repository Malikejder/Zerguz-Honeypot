...
class ZerguzFirewallManager:
    ...
    def block_ip(self, ip: str) -> bool:
        ...
        try:
            if self.os_type == "Linux":
                ...
                self.logger.warning(f"Unsupported operating system: {self.os_type}")
                return False
            if success:
            ...
            self.logger.error(f"Firewall exception: {error}")
            self.logger.log_error(f"Firewall exception for {ip}: {error}")
            return False
    ...
    def _block_linux(self, ip: str) -> bool:
        if shutil.which("ufw"):
            ...
            self.logger.warning("No supported Linux firewall found.")
            return False
    ...
    def _block_windows(self, ip: str) -> bool:
        command = ["netsh", "advfirewall", "firewall", "add", "rule",
                   f"name=ZERGUZ_BLOCK_{ip}", "dir=in", "action=block", f"remoteip={ip}"]
        result = subprocess.run(command, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
        return result.returncode == 0
